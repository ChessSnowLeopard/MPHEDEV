...
// ComputeCrossEntropyLossPrivacyPreserving 计算隐私保护的交叉熵损失
// 全程在密文域计算，不泄露任何明文信息
func (cel *CrossEntropyLoss) ComputeCrossEntropyLossPrivacyPreserving(
	...
	// 步骤2：计算log(P^j)
	logOutputs := make([]*rlwe.Ciphertext, len(softmaxOutputs))
	for j := 0; j < len(softmaxOutputs); j++ {
		logResult, err := cel.LogApproximator.Apply(softmaxOutputs[j], evaluator, params)
		if err != nil {
			return nil, fmt.Errorf("计算log失败(类别%d): %v", j, err)
		}
		logOutputs[j] = logResult
	}

	// 步骤3：计算Y^j ⊙ log(P^j)
	weightedLogs := make([]*rlwe.Ciphertext, len(logOutputs))
	for j := 0; j < len(logOutputs); j++ {
		weighted, err := evaluator.MulNew(logOutputs[j], oneHotLabels[j])
		if err != nil {
			return nil, fmt.Errorf("计算加权log失败(类别%d): %v", j, err)
		}
		if err := evaluator.Rescale(weighted, weighted); err != nil {
			return nil, fmt.Errorf("Rescale失败(类别%d): %v", j, err)
		}
		weightedLogs[j] = weighted
	}

	// 步骤4：计算sum(Y^j ⊙ log(P^j))
	sumLoss := weightedLogs[0].CopyNew()
	for j := 1; j < len(weightedLogs); j++ {
		if err := evaluator.Add(sumLoss, weightedLogs[j], sumLoss); err != nil {
			return nil, fmt.Errorf("累加损失失败: %v", err)
		}
	}

	// 步骤4.5：取负号（交叉熵损失为负对数似然）
	if err := evaluator.MulRelin(sumLoss, -1.0, sumLoss); err != nil {
		return nil, fmt.Errorf("取负号失败: %v", err)
	}

	// 步骤5：计算批次平均损失
	avgLoss, err := cel.computeBatchAverageEfficientPrivacy(sumLoss, evaluator, params)
	if err != nil {
		return nil, fmt.Errorf("计算批次平均失败: %v", err)
	}
