...
func (mlp *MLP) ForwardPropagation(
	...
	// 第一层：从原始打包格式计算
	fmt.Printf("\n处理第1层（输入层 -> 隐藏层）...\n")
	fmt.Printf("激活函数: %s\n", mlp.Layers[0].Activation.GetName())
	...
	for i := 1; i < len(mlp.Layers)-1; i++ {
		fmt.Printf("\n处理第%d层...\n", i+1)
		fmt.Printf("激活函数: %s\n", mlp.Layers[i].Activation.GetName())
	...
	if len(mlp.Layers) > 1 {
		fmt.Printf("\n处理输出层...\n")
		fmt.Printf("激活函数: %s\n", mlp.Layers[len(mlp.Layers)-1].Activation.GetName())
	...
	fmt.Printf("\n✓ 前向传播完成，总用时: %v\n", duration)
	fmt.Printf("• 层数: %d\n", len(mlp.Layers))
	fmt.Printf("• 平均每层用时: %v\n", duration/time.Duration(len(mlp.Layers)))
...
func (mlp *MLP) ForwardPropagationWithIntermediates(
	...
	// 第一层：从原始打包格式计算（输入层 -> 隐藏层）
	fmt.Printf("\n处理第1层（输入层 -> 隐藏层）...\n")
	fmt.Printf("激活函数: %s\n", mlp.Layers[0].Activation.GetName())

	// 计算隐藏层的激活前输入和激活后输出
	hiddenInputs, hiddenActivations, err := mlp.ComputeHiddenLayerWithIntermediates(
		encryptedVectors, batchIdx, params, evaluator, encoder, slots)
	if err != nil {
	...
	currentOutputs = hiddenActivations
	...
	if len(mlp.Layers) > 1 {
		...
		// 只有一层的情况，隐藏层就是输出层
		results.FinalOutputs = currentOutputs
	}

	results.ComputationTime = time.Since(startTime)
	fmt.Printf("\n✓ 前向传播（含中间结果）完成，总用时: %v\n", results.ComputationTime)
	fmt.Printf("• 层数: %d\n", results.LayerCount)
	fmt.Printf("• 隐藏层激活前输入: %d 个密文\n", len(results.HiddenInputs))
	fmt.Printf("• 隐藏层激活后输出: %d 个密文\n", len(results.HiddenActivations))
	fmt.Printf("• 最终输出: %d 个密文\n", len(results.FinalOutputs))

	return results, nil
}

// ComputeHiddenLayerWithIntermediates 计算隐藏层并保存激活前和激活后的结果
func (mlp *MLP) ComputeHiddenLayerWithIntermediates(
	...
	hiddenInputs, err := mlp.computeLayerWithoutActivation(
		hiddenLayer, encryptedVectors, batchIdx, params, evaluator, encoder, slots)
	if err != nil {
		return nil, nil, fmt.Errorf("计算激活前输入失败: %v", err)
	}

	// 计算激活后的输出（应用激活函数并重组）
	hiddenActivations, err := hiddenLayer.ComputeEncryptedWithReorganization(
		encryptedVectors, batchIdx, params, evaluator, encoder, slots)
	if err != nil {
		return nil, nil, fmt.Errorf("计算激活后输出失败: %v", err)
	}

	fmt.Printf("✓ 隐藏层中间结果保存完成\n")
	fmt.Printf("• 激活前输入: %d 个神经元输出\n", len(hiddenInputs))
	fmt.Printf("• 激活后输出: %d 个重组后向量\n", len(hiddenActivations))

	return hiddenInputs, hiddenActivations, nil
}
