...
// ComputeOutputLayerGradients 计算输出层梯度
...
func (oge *OutputGradientEngine) ComputeOutputLayerGradients(
	...
	if len(labels) < oge.BatchSize {
		return nil, fmt.Errorf("标签数量 (%d) 少于批大小 (%d)", len(labels), oge.BatchSize)
	}

	// 步骤1: 创建one-hot编码标签
	fmt.Println("\n📊 步骤1：创建one-hot编码标签")
	oneHotLabels, err := oge.createOneHotLabels(labels, params, encoder)
	if err != nil {
		return nil, fmt.Errorf("创建one-hot标签失败: %v", err)
	}

	// 步骤2: 计算梯度 δ^j = P^j - Y^j
	fmt.Println("\n📊 步骤2：计算输出层梯度 δ^j = P^j - Y^j")
	...
	for j := 0; j < oge.NumClasses; j++ {
		...
		// δ^j = P^j - Y^j
		gradient, err := evaluator.SubNew(softmaxOutputs[j], oneHotLabels[j])
		if err != nil {
...
// VerifyOutputLayerGradients 验证输出层梯度计算精度
func (oge *OutputGradientEngine) VerifyOutputLayerGradients(
	...
	// 计算明文梯度进行对比
	fmt.Println("\n📊 梯度精度对比（前5个样本）：")
	fmt.Println("样本 | 类别 | 真实标签 | Softmax值 | 明文梯度 | 密文梯度 | 绝对误差 | 相对误差")
	fmt.Println("-----|------|----------|-----------|----------|----------|----------|----------")
	...
	for m := 0; m < min(5, oge.BatchSize) && m < len(labels); m++ {
		...
		for j := 0; j < oge.NumClasses; j++ {
			...
			fmt.Printf("%4d | %4d | %8d | %9.6f | %8.6f | %8.6f | %8.6f | %7.3f%% %s\n",
				m, j, trueLabel, pValue, plaintextGradient, ciphertextGradient, absError, relError, marker)
		}
	}
	...
	// 显示梯度特性
	fmt.Printf("\n📈 梯度特性分析:\n")
	for m := 0; m < min(3, oge.BatchSize) && m < len(labels); m++ {
		trueLabel := int(labels[m])
		fmt.Printf("  样本%d (真实标签: %d):\n", m, trueLabel)
