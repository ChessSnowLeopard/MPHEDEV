...
type TrainingEpochResult struct {
	Epoch        int     `json:"epoch"`        // 轮次
	Loss         float64 `json:"loss"`         // 损失值
	Accuracy     float64 `json:"accuracy"`     // 准确率
	LearningRate float64 `json:"learningRate"` // 学习率
	EpochTime    string  `json:"epochTime"`    // 本轮用时（字符串格式）
	Timestamp    int64   `json:"timestamp"`    // 时间戳
}

// TrainingStatus 训练状态
type TrainingStatus struct {
	Status      string               `json:"status"`      // 训练状态：idle, training, completed
	TotalEpochs int                  `json:"totalEpochs"` // 总轮数
	LastEpoch   *TrainingEpochResult `json:"lastEpoch"`   // 最新轮次结果
}
...
// AddTrainingEpoch 添加训练轮次结果
func AddTrainingEpoch(epoch int, loss, accuracy, learningRate float64, epochTime time.Duration) {
	trainingMutex.Lock()
	defer trainingMutex.Unlock()

	epochResult := TrainingEpochResult{
		Epoch:        epoch,
		Loss:         loss,
		Accuracy:     accuracy,
	...
	trainingStatus.TotalEpochs = len(trainingHistory)
	trainingStatus.LastEpoch = &epochResult

	fmt.Printf("📊 添加训练轮次: Epoch=%d, Loss=%.6f, Accuracy=%.2f%%, LR=%.6f, Time=%v\n",
		epoch, loss, accuracy*100, learningRate, epochTime)
}

// SetTrainingCompleted 设置训练完成状态
func SetTrainingCompleted() {
	trainingMutex.Lock()
	defer trainingMutex.Unlock()

	trainingStatus.Status = "completed"
	fmt.Printf("✅ 训练完成，总轮数: %d\n", len(trainingHistory))
}

// GetTrainingHistory 获取训练历史
func GetTrainingHistory() []TrainingEpochResult {
	trainingMutex.RLock()
	defer trainingMutex.RUnlock()

	// 返回副本，避免并发访问问题
	history := make([]TrainingEpochResult, len(trainingHistory))
	copy(history, trainingHistory)
	return history
}
...
// getTrainingHistoryHandler 获取训练历史处理器
func (c *Coordinator) getTrainingHistoryHandler(ctx *gin.Context) {
...
// getTrainingStatusHandler 获取训练状态处理器
func (c *Coordinator) getTrainingStatusHandler(ctx *gin.Context) {
...
func (c *Coordinator) parseTrainingOutput(line string) {
	...
	if strings.Contains(line, "✅") && strings.Contains(line, "轮训练完成") {
		...
		// 提取损失值
		lossMatch := regexp.MustCompile(`损失=([\d.]+)`).FindStringSubmatch(line)
		if len(lossMatch) < 2 {
			return
		}
		loss, err = strconv.ParseFloat(lossMatch[1], 64)
		...
		// 提取准确率
		accuracyMatch := regexp.MustCompile(`准确率=([\d.]+)%`).FindStringSubmatch(line)
		if len(accuracyMatch) < 2 {
			return
		}
		accuracy, err = strconv.ParseFloat(accuracyMatch[1], 64)
