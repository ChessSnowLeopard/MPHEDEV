...
type Handlers struct {
	...
	participant       interface{ GetOutputBuffer() *buffer.OutputBuffer } // 添加参与方实例引用
}

// NewHandlers 创建新的处理器集合
func NewHandlers(keyManager *crypto.KeyManager, decryptionService *crypto.DecryptionService, refreshService *crypto.RefreshService, participant interface{ GetOutputBuffer() *buffer.OutputBuffer }) *Handlers {
	return &Handlers{
...
func (h *Handlers) handleCtCNNOutput(w http.ResponseWriter, r *http.Request) {
	...
	// 将ctCNN输出添加到参与方的OutputBuffer中
	if h.participant != nil {
		outputBuffer := h.participant.GetOutputBuffer()
		if outputBuffer != nil {
			// 根据输出类型选择日志级别
			switch output.Type {
			case "error":
				outputBuffer.AddError(output.Content)
			case "warning":
				outputBuffer.AddWarning(output.Content)
			case "debug":
				outputBuffer.AddDebug(output.Content)
			default:
				outputBuffer.AddInfo(output.Content)
			}
		}
	}
