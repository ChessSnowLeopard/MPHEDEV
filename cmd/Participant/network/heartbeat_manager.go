...
func NewHeartbeatManager(coordinatorURL string, client *types.HTTPClient) *HeartbeatManager {
...
func (hm *HeartbeatManager) StartHeartbeat(participantID int) {
	hm.heartbeatTicker = time.NewTicker(5 * time.Second)

	go func() {
		for {
			select {
			case <-hm.heartbeatTicker.C:
				hm.sendHeartbeat(participantID)
...
// sendHeartbeat 发送心跳到协调器
func (hm *HeartbeatManager) sendHeartbeat(participantID int) {
	reqBody, _ := json.Marshal(map[string]int{
		"participant_id": participantID,
	})

	resp, err := hm.client.Client.Post(hm.coordinatorURL+"/heartbeat", "application/json", bytes.NewReader(reqBody))
	if err != nil {
		if !hm.silentMode {
			fmt.Printf("心跳发送失败: %v\n", err)
		}
	...
	if resp.StatusCode != http.StatusOK && !hm.silentMode {
		fmt.Printf("心跳响应异常: %d\n", resp.StatusCode)
	}
}
